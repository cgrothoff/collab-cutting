...
class RobotControl(object):
    def __init__(self, rosrate):
        ...
        # print("[CLIENT] Connected to hand detection server")
        ...
        self.zero_quaternion = self.R.euler2quat(np.deg2rad(-178.670), 
                                                 np.deg2rad(-0.155), 
                                                 np.deg2rad(-90.))
        self.knife_offset_vector = np.array([0.03, -0.26])
    ...
    def read_force(self):
        force = np.array(self.wrench_fb[:3])
        return force
    
    def read_torque(self):
        torque = np.array(self.wrench_fb[3:])
        return torque
    
    def go2goal(self, goal=None, socket_hand=None, socket_force=None, max_time=15., mode=None, joint_space=False):
        # Initialize joystick
        ...
        traj = [[0.] + self.read_pose()[:3].tolist() + goal[3:],
                [max_time] + goal]
        ...
        while not done:
            ...
            if socket_hand is not None:
                readable, _, _ = select.select([socket_hand], [], [], 0) 
                for sock in readable: 
                    try:
                        ...
                        for msg in messages: 
                            ...
                            if parsed.get("safety") == "hand_detected": 
                                print("[!] Cut Execution Stopped: Hand detected in workspace") 
                                self.stop()
                                trigger = True
                                sock.sendall(json.dumps({"status": "interrupted"}).encode("utf-8")) 
                                # return False 
                    
                            # elif parsed.get("safety") == "force_triggered": 
                            #     print("[!] Cut Execution Stopped: Force exceeded") 
                            #     self.stop() 
                            #     trigger = True
                            #     sock.sendall(json.dumps({"status": "interrupted"}).encode("utf-8")) 
                            #     return False 
                                ...
                                self.resume()
                                trigger = False
                                sock.sendall(json.dumps({"status": "acknowledged"}).encode("utf-8")) 
                    
                    except Exception as e: 
                        print("[SERVER ERROR]", e) 
                        return False 
            
            if socket_force is not None:
                readable, _, _ = select.select([socket_force], [], [], 0) 
                for sock in readable: 
                    try:
                        data = sock.recv(4096) 
                        if not data: continue 
                        ...
                        for msg in messages: 
                            ...
                            if parsed.get("safety") == "force_triggered": 
                                print("[!] Cut Execution Stopped: Force exceeded") 
                                self.stop() 
                                trigger = True
                                sock.sendall(json.dumps({"status": "interrupted"}).encode("utf-8")) 
    ...
    def follow_traj(self, times, traj, socket_hand, socket_force, trajname=None):
        # Initialize joystick
        self.joystick = Joystick()
        ...
        for idx in range(len(traj) - 1):
            traj_fcns.append(Trajectory(traj[idx:idx+2, :]))
        ...
        goal = np.zeros(7)
        goal[:3] = traj_fcns[0].get_waypoint(t)[:3]
        goal[3:] = robot_orientation.copy()
        self.go2goal(goal, socket_hand=socket_hand, socket_force=socket_force, max_time=10., mode='compliance')
        self.switch_controller(mode='compliance')
        
        
        for segment_idx, traj_fcn in enumerate(traj_fcns):
            ''' ############################################################### '''
            ''' ############################################################### '''
            ''' Get the XY correction and new orientation for each traj segment '''
            ''' ############################################################### '''
            ''' ############################################################### '''
            # trajectory segment orientation
            delta_yaw = traj_fcn.get_segment_orientation() - np.pi / 2.
            delta_yaw_quat = self.R.euler2quat(0., 0., delta_yaw)
            new_orientation = self.R.quat_multiply(delta_yaw_quat, self.zero_quaternion)         
            
            # trajectory segment error correction
            self.prev_XY = self.read_pose()[:2]
            start_waypoint = traj_fcn.get_waypoint(times[segment_idx])
            correction_XY = self.correct_course(start_waypoint, delta_yaw, new_orientation, socket_hand, socket_force)
            ...
            while not done:
                ...
                if not trigger:
                    des_s = np.zeros(7)
                    # orientation
                    des_s[3:] = new_orientation.copy()
                    # position
                    waypoint = np.array(traj_fcn.get_waypoint(t))
                    waypoint[0] += correction_XY[0]
                    waypoint[1] += correction_XY[1]
    ...
    def correct_course(self, waypoint, delta_yaw, new_orientation, socket_hand, socket_force):
        raised_height = 0.3
        time_allowed = 2.5
        x_offset = 0.02
        
        # calculate the raised pose
        current_pose = self.read_pose()
        raised_pose = current_pose.copy()
        raised_pose[2] = raised_height
        self.go2goal(raised_pose, socket_hand=socket_hand, socket_force=socket_force, max_time=time_allowed)
        ...
        # delta_vector[0] += x_offset
        waypoint[0] += delta_vector[0]
        waypoint[1] += delta_vector[1]
        waypoint = constrain_wayp(waypoint)
        correct_pose[:2] = waypoint[:2]
        correct_pose[2] = raised_height
        self.go2goal(correct_pose, socket_hand=socket_hand, socket_force=socket_force, max_time=time_allowed)
        
        # calculate the lower pose
        current_pose = self.read_pose()
        lowered_pose = current_pose.copy()
        lowered_pose[2] = waypoint[2]
        self.go2goal(lowered_pose, socket_hand=socket_hand, socket_force=socket_force, max_time=time_allowed)
                    
        return delta_vector
            
    def send(self, joystick_action, initial_pose, mode_rotation):
        xdot, ydot, zdot, _, _, _ = joystick_action
        ...
        if not mode_rotation:
            ...
            adot, bdot, cdot, _, _, _ = joystick_action
            adot *= 5
            bdot *= 5
            cdot *= 5
            ...
            orientation = self.R.quat_multiply(yaw_quat, self.read_pose()[3:])
            new_pose[3:] = orientation.copy()
            feedback_pose[3:] = orientation.copy()
